...
class InlineHelper
{
    ...
    public function __construct(\MASK\Mask\Domain\Repository\StorageRepository $storageRepository = null)
    {
        if (!$storageRepository) {
            $this->storageRepository = GeneralUtility::makeInstance(\MASK\Mask\Domain\Repository\StorageRepository::class);
        } else {
        ...
        if ($elementFields) {
            ...
            foreach ($elementFields as $field) {
                ...
                if ($type === 'Inline') {
                    ...
                    $elements = $this->getInlineElements($data, $fieldKeyPrefix, $cType, $fieldKeyPrefix . '_parent', 'tt_content', 'tt_content');
                    $data[$fieldKeyPrefix] = $elements;
                }
            }
        }
